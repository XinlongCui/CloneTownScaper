...
namespace TS
{
    public class Coord
    {
        ...
        { 
            ...
            worldPosition = new Vector3(q + 0.5f * r, 0, -Mathf.Sqrt(3) / 2 * r) * GridManager.cellSize; //new Vector3(q * Mathf.Sqrt(3) / 2, 0, -(float)r - ((float)q / 2)) * 2 * GridManager.cellSize; //new Vector3(q+0.5f*r, 0, Mathf.Sqrt(3) /2*r) * GridManager.cellSize;
        }
